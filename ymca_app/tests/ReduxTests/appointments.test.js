...
describe('appointment test suite', () => {
  ...
  test('can add appointment', () => {
    const appointment = {_id: 100, menteeid: 12, date: "09/01/2019"}
    store.dispatch(addAppointment(appointment))

    let state = store.getState()

    expect(state.appointments[0]).not.toBeNull()
    expect(state.appointments[0]._id).toBe(100)
  ...
  test('can set appointments', () => {
    const appointments = [
      {_id: 100, menteeid: 12, date: "09/01/2019"},
      {_id: 200, menteeid: 13, date: "12/01/2019"}
    ]
    ...
    expect(state.appointments[1]._id).toBe(200)
  ...
  test('can remove appointments', () => {
    ...
    expect(store.getState().appointments[0]._id).toBe(200)
  ...
  test('can update appointments', () => {
    ...
    store.dispatch(updateAppointment(100, {_id: 100, menteeid: 23, date: "09/02/2019"}))
    expect(store.getState().appointments[0]).not.toBeNull()
    expect(store.getState().appointments[0]._id).toBe(100)
    expect(store.getState().appointments[0].menteeid).toBe(23)
    expect(store.getState().appointments[0].date).toBe("09/02/2019")

    store.dispatch(updateAppointment(200, {_id: 200, menteeid: 14, date: "11/01/2019"}))
    expect(store.getState().appointments[1]).not.toBeNull()
    expect(store.getState().appointments[1]._id).toBe(200)
